        ...
        <div className="hidden lg:flex items-stretch justify-center gap-0">
          {CompanyFeatures.map((feature, index) => (
            <motion.div
              ...
              className={`flex flex-col items-center justify-start gap-5 py-8 px-4 border-2 border-[#EEE] ${
                index === 0
                  ? "rounded-l-[10px] border-r-0 flex-[1.8]"
                  : index === CompanyFeatures.length - 1
                  ? "rounded-r-[10px] flex-1"
                  : "border-r-0 flex-1"
              }`}
              style={{ backgroundColor: feature.bgColor }}
            >
              <NetworkIcon />
              <h3 className={`${feature.titleSize} font-bold text-[#456FF6] text-center leading-tight`}>
                {feature.title}
              </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 lg:hidden">
          {CompanyFeatures.map((feature, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              transition={{ delay: index * 0.1 }}
              className="flex flex-col items-center justify-start gap-5 p-6 border-2 border-[#EEE] rounded-lg"
              style={{ backgroundColor: index === 0 ? "#E7F2FF" : "#FFF" }}
            >
              <NetworkIcon />
              <h3 className="text-xl font-bold text-[#456FF6] text-center leading-tight">
                {feature.title}
              </h3>
